...
export const LEGAL_PAGE_RENDER_LOCALES = [
  "hu",
  "en",
  "de",
...
export type LegalPageLocale = (typeof LEGAL_PAGE_RENDER_LOCALES)[number];
export type LegalPageSlug = (typeof LEGAL_PAGE_SLUGS)[number];

export function isLegalPageLocale(locale: string): locale is LegalPageLocale {
  return (LEGAL_PAGE_RENDER_LOCALES as readonly string[]).includes(locale);
}

export function legalPageStaticParams() {
  return LEGAL_PAGE_RENDER_LOCALES.map((locale) => ({ locale }));
}
