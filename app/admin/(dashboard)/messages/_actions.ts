...
import { isLeadStatus } from "@/lib/messages-lead";
...
function normalizeOptionalText(
  ...
  if (trimmed.length > maxLength) {
    return {
      ok: false,
      error: `${label} tul hosszu (max. ${maxLength} karakter).`,
    };
  }
...
function normalizeOptionalUrl(
  ...
  if (trimmed.length > 2_000) {
    return {
      ok: false,
      error: `${label} tul hosszu (max. 2000 karakter).`,
    };
  }
  try {
    const parsed = new URL(trimmed);
    if (parsed.protocol !== "https:" && parsed.protocol !== "http:") {
      return { ok: false, error: `${label} csak http vagy https link lehet.` };
    }
  } catch {
    return { ok: false, error: `${label} ervenytelen link.` };
  }
  return { ok: true, value: trimmed };
}
...
export async function updateLeadDetails(
  ...
  if (!Number.isInteger(messageId) || messageId <= 0) {
    return { ok: false, error: "Ervenytelen uzenet azonosito." };
  }

  const leadStatus = input.leadStatus.trim();
  if (!isLeadStatus(leadStatus)) {
    return { ok: false, error: "Ervenytelen lead statusz." };
  }

  const owner = normalizeOptionalText(input.leadOwnerName, 120, "Felelos");
  if (!owner.ok) return owner;

  const nextActionNote = normalizeOptionalText(
    input.leadNextActionNote,
    2_000,
    "Kovetkezo teendo",
  );
  if (!nextActionNote.ok) return nextActionNote;

  const siteType = normalizeOptionalText(input.leadSiteType, 80, "Helyszintipus");
  if (!siteType.ok) return siteType;

  const proposalUrl = normalizeOptionalUrl(input.leadProposalUrl, "Ajanlat link");
  if (!proposalUrl.ok) return proposalUrl;

  const contractUrl = normalizeOptionalUrl(
    input.leadContractUrl,
    "Szerzodes link",
  );
  if (!contractUrl.ok) return contractUrl;
  ...
  const rawNextActionAt = input.leadNextActionAt.trim();
  if (rawNextActionAt) {
    nextActionAt = new Date(rawNextActionAt);
    if (Number.isNaN(nextActionAt.getTime())) {
      return { ok: false, error: "Ervenytelen kovetkezo teendo idopont." };
    }
  }

  let estimatedValue: number | null = null;
  const rawEstimatedValue = input.leadEstimatedValue
    .trim()
    .replace(/\s/g, "")
    .replace(/[^\d]/g, "");
  if (rawEstimatedValue) {
    estimatedValue = Number.parseInt(rawEstimatedValue, 10);
    if (
      !Number.isSafeInteger(estimatedValue) ||
      estimatedValue < 0 ||
      estimatedValue > 2_000_000_000
    ) {
      return { ok: false, error: "Ervenytelen becsult ertek." };
    }
  }
  ...
  try {
    await db
      ...
      .set({
        leadStatus,
        leadOwnerName: owner.value,
        leadNextActionAt: nextActionAt,
        leadNextActionNote: nextActionNote.value,
        leadEstimatedValue: estimatedValue,
        leadSiteType: siteType.value,
        leadProposalUrl: proposalUrl.value,
        leadContractUrl: contractUrl.value,
        internalNotes: internalNotes.value,
        leadUpdatedAt: new Date(),
      })
    ...
    return {
      ...
      error:
        err instanceof Error
          ? err.message
          : "A lead adatok mentese sikertelen.",
    };
  }
